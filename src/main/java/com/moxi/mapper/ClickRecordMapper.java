...
public interface ClickRecordMapper {
	...
	@Select("select * from click_record_back where id = #{id}")
	...
	@Select({
		...
		"FROM click_record cr WHERE cr.app_id = #{appId} AND date_format(cr.create_time, '%Y-%m-%d') = #{date} GROUP BY timeDay ) a ",
		"LEFT JOIN ( ",
		...
		"FROM activation_record ar JOIN click_record cr ON cr.id = ar.click_id ",
	...
	@Select({
		...
		"FROM click_record cr LEFT JOIN channel c on cr.channel_code = c.code WHERE cr.app_id = #{appId} AND date_format(cr.create_time, '%Y-%m-%d') = #{date} GROUP BY c.code,timeDay ) a ",
		"LEFT JOIN ( ",
		...
		"FROM activation_record ar JOIN click_record cr ON cr.id = ar.click_id LEFT JOIN channel c on cr.channel_code = c.code ",
